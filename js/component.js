...
$.ajaxSetup({
    ...
    error: function(x, e){
        ...
        {
        	// alert('您的访问次数太多了，休息一会再试吧');
        	popupalert({
				type: 'error',
				content: '您的访问次数太多了，休息一会再试吧',
				offset: 100,
				animate: 'svgDash',
			});
        } 
        else 
        {
            ...
            popupalert({
				type: 'error',
				content: '未知错误：'+x.status + x.responseText,
				offset: 100,
				animate: 'svgDash',
			});
        }
...
function myAjax( ajaxMethod , ajaxUrl , ajaxArray , params)
{
    $.ajax({
        ...
        beforeSend: function(XMLHttpRequest){
        	if(params['start'])
        		params['start'](XMLHttpRequest);
        },
        success: function(response, textStatus, jqXHR){

           	if(response.msgSuccess)
	        {
	        	popupalert({
					type: 'success',
					content: response.msgSuccess,
					offset: 100,
					animate: 'svgDash',
				});
	        }
	        ...
	        if( response.status == params['status'] )
	        {
	            ...
	            if(params['attr'])
	                params['attr'].attr(response.attrName, response.attrValue);
